...
function usage() {
  cat << EOF
$0 :: create Vault Policies governing access to Tanzu Sync and TAP Install secrets
...
VAULT_SECRET_PATH_FOR_TANZU_SYNC_WITH_DATA=${VAULT_SECRET_PATH_FOR_TANZU_SYNC_WITH_DATA:-"secret/data/dev/${CLUSTER_NAME}/tanzu-sync"}
VAULT_SECRET_PATH_FOR_TAP_WITH_DATA=${VAULT_SECRET_PATH_FOR_TAP_WITH_DATA:-"secret/data/dev/${CLUSTER_NAME}/tap"}
...
path "${VAULT_SECRET_PATH_FOR_TANZU_SYNC_WITH_DATA}/*" {
  capabilities = ["read"]
}
EOF
...
path "${VAULT_SECRET_PATH_FOR_TAP_WITH_DATA}/*" {
  capabilities = ["read"]
}
EOF
